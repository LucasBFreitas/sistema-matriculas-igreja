...
function App(){
 ...
 async function carregarDados(){setLoading(true);const [al,cur,tur,prof,mat,per,pre,usu]=await Promise.all([supabase.from('alunos').select('*, responsaveis(nome, cpf, telefone, email, endereco)').order('created_at',{ascending:false}),supabase.from('cursos').select('*').order('nome'),supabase.from('turmas').select('*, cursos(nome), professores(nome)').order('created_at',{ascending:false}),supabase.from('professores').select('*').order('nome'),supabase.from('matriculas').select('*, alunos(nome, ativo, excluido), turmas(nome, cursos(nome), professores(nome)), periodos_letivos(nome, data_inicio, data_fim)').order('created_at',{ascending:false}),supabase.from('periodos_letivos').select('*').order('nome',{ascending:false}),supabase.from('presencas').select('*'),supabase.from('usuarios_perfis').select('*, professores(nome)').order('email')]); if(al.error||cur.error||tur.error||prof.error||mat.error||per.error||pre.error)setErro(al.error?.message||cur.error?.message||tur.error?.message||prof.error?.message||mat.error?.message||per.error?.message||pre.error?.message); else{setAlunos(al.data||[]);setCursos(cur.data||[]);setTurmas(tur.data||[]);setProfessores(prof.data||[]);setMatriculas(mat.data||[]);setPeriodosLetivos(per.data||[]);setPresencas(pre.data||[]);setUsuarios(usu.data||[]);const perfil=(usu.data||[]).find(u=>u.email?.toLowerCase()===session?.user?.email?.toLowerCase());setPerfilUsuario(perfil?.perfil||'administrador')} setLoading(false)}
...
async function confirmarRenovacao(e){
 ...
 const abas=[['painel','🏠 Painel'],['alunos','🎓 Alunos'],['professores','👨‍🏫 Professores'],['cursos','📚 Cursos'],['periodos','🗓️ Períodos'],['turmas','🏫 Turmas'],['inscricoes','📝 Inscrições'],['presencas','✅ Presenças'],['relatorios','📊 Relatórios']]
