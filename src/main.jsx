...
function App(){
 ...
 const [alunos,setAlunos]=useState([]),[cursos,setCursos]=useState([]),[turmas,setTurmas]=useState([]),[professores,setProfessores]=useState([]),[matriculas,setMatriculas]=useState([]),[periodosLetivos,setPeriodosLetivos]=useState([])
 const [alunoForm,setAlunoForm]=useState(alunoVazio),[alunoEditId,setAlunoEditId]=useState(null),[profForm,setProfForm]=useState(professorVazio),[profEditId,setProfEditId]=useState(null),[cursoForm,setCursoForm]=useState(cursoVazio),[cursoEditId,setCursoEditId]=useState(null),[turmaForm,setTurmaForm]=useState(turmaVazia),[turmaEditId,setTurmaEditId]=useState(null),[matriculaForm,setMatriculaForm]=useState({aluno_id:'',turma_id:'',periodo_letivo_id:'',observacoes:''}),[renovacaoModal,setRenovacaoModal]=useState(null),[renovacaoForm,setRenovacaoForm]=useState({periodo_letivo_id:'',manter_turma:true,turma_id:''}),[periodoForm,setPeriodoForm]=useState(periodoLetivoVazio),[periodoEditId,setPeriodoEditId]=useState(null)
 ...
 async function carregarDados(){setLoading(true);const [al,cur,tur,prof,mat,per]=await Promise.all([supabase.from('alunos').select('*, responsaveis(nome, cpf, telefone, email, endereco)').order('created_at',{ascending:false}),supabase.from('cursos').select('*').order('nome'),supabase.from('turmas').select('*, cursos(nome), professores(nome)').order('created_at',{ascending:false}),supabase.from('professores').select('*').order('nome'),supabase.from('matriculas').select('*, alunos(nome, ativo, excluido), turmas(nome, cursos(nome), professores(nome)), periodos_letivos(nome, data_inicio, data_fim)').order('created_at',{ascending:false}),supabase.from('periodos_letivos').select('*').order('nome',{ascending:false})]); if(al.error||cur.error||tur.error||prof.error||mat.error||per.error)setErro(al.error?.message||cur.error?.message||tur.error?.message||prof.error?.message||mat.error?.message||per.error?.message); else{setAlunos(al.data||[]);setCursos(cur.data||[]);setTurmas(tur.data||[]);setProfessores(prof.data||[]);setMatriculas(mat.data||[]);setPeriodosLetivos(per.data||[])} setLoading(false)}
...
async function confirmarRenovacao(e){
 ...
 function linhaRelatorioAluno(a){
  ...
  return {id:a.id,aluno:a.nome||'-',turma:m?`${m.turmas?.cursos?.nome||''} - ${m.turmas?.nome||''}`:'-',periodo:m?.periodos_letivos?.nome||'-',professor:m?.turmas?.professores?.nome||'-'}
 }
 const relatorio=useMemo(()=>{
  if(relatorioTurma.startsWith('turma:')){
    const turmaId=relatorioTurma.replace('turma:','')
    return matriculasValidas.filter(m=>m.turma_id===turmaId).map(m=>({id:m.id,aluno:m.alunos?.nome||'-',turma:`${m.turmas?.cursos?.nome||''} - ${m.turmas?.nome||''}`,periodo:m.periodos_letivos?.nome||'-',professor:m.turmas?.professores?.nome||'-'}))
  }
  ...
  return matriculasValidas.map(m=>({id:m.id,aluno:m.alunos?.nome||'-',turma:`${m.turmas?.cursos?.nome||''} - ${m.turmas?.nome||''}`,periodo:m.periodos_letivos?.nome||'-',professor:m.turmas?.professores?.nome||'-'}))
 ...
 const abas=[['painel','🏠 Painel'],['alunos','🎓 Estudantes'],['professores','👨‍🏫 Professores'],['cursos','📚 Cursos'],['periodos','🗓️ Períodos'],['turmas','🏫 Turmas'],['inscricoes','📝 Inscrições'],['relatorios','📊 Relatórios']]
