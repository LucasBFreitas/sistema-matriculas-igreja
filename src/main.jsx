...
async function confirmarRenovacao(e){
  ...
  const turma=turmasVisiveis.find(t=>t.id===turmaId)
 ...
 async function cancelarMatricula(id){if(isProfessor)return falhar('Professor pode apenas visualizar inscrições.');if(!confirm('Cancelar esta inscrição?'))return;const {error}=await supabase.from('matriculas').update({status:'cancelada'}).eq('id',id);if(error)return falhar(error.message);await atualizar('Inscrição cancelada.')}
 ...
 const perfilAtual=usuarios.find(u=>u.email?.toLowerCase()===session?.user?.email?.toLowerCase())
 const professorIdAtual=perfilAtual?.professor_id||null
 const turmasVisiveis=isProfessor&&professorIdAtual?turmas.filter(t=>String(t.professor_id)===String(professorIdAtual)):turmas
 const turmaIdsVisiveis=turmasVisiveis.map(t=>String(t.id))
 const matriculasVisiveis=isProfessor&&professorIdAtual?matriculas.filter(m=>turmaIdsVisiveis.includes(String(m.turma_id))):matriculas
 const presencasVisiveis=isProfessor&&professorIdAtual?presencas.filter(p=>turmaIdsVisiveis.includes(String(p.turma_id))):presencas
 function podeVerAba(id){
  if(isAdmin)return true
  if(isCoordenador)return true
  if(isProfessor)return ['painel','turmas','inscricoes','presencas','relatorios'].includes(id)
  return id==='painel'
 }

 const listasPresenca=Object.values(presencasVisiveis.reduce((acc,p)=>{
 ...
 function abrirListaPresenca(lista){
  const regs=presencasVisiveis.filter(p=>p.turma_id===lista.turma_id&&p.periodo_letivo_id===lista.periodo_letivo_id&&p.data_chamada===lista.data_chamada)
  const mapa={}
 ...
 const alunosChamada=matriculasVisiveis.filter(m=>m.status!=='cancelada'&&m.alunos&&m.turma_id===chamadaForm.turma_id&&String(m.periodo_letivo_id||'')===String(chamadaForm.periodo_letivo_id||'')).sort((a,b)=>(a.alunos?.nome||'').localeCompare(b.alunos?.nome||''))
 function marcarPresenca(matriculaId,status){setChamada({...chamada,[matriculaId]:status})}
 function totalPresencasAluno(alunoId){return presencasVisiveis.filter(p=>p.aluno_id===alunoId&&p.status==='presente').length}
 ...
 const inscricoesFiltradas=matriculasVisiveis.filter(m=>{
 ...
 const matriculasValidas=matriculasVisiveis.filter(m=>m.status!=='cancelada'&&m.alunos&&!m.alunos.excluido&&m.alunos.ativo!==false)
 ...
 },[relatorioTurma,alunos,matriculasVisiveis,turmasVisiveis,presencasVisiveis])
 function imprimirRelatorio(tipo='1'){if(!relatorioPermitido())return falhar('Professor só pode emitir relatórios das suas turmas.');
 ...
 const turmasAtivas=turmasVisiveis.length
 const matriculasAtivas=matriculasVisiveis.filter(m=>m.status!=='cancelada'&&m.alunos&&!m.alunos.excluido&&m.alunos.ativo!==false).length
 const vagasTotais=turmasVisiveis.reduce((acc,t)=>acc+Number(t.vagas||0),0)
 const vagasOcupadasTotal=turmasVisiveis.reduce((acc,t)=>acc+vagasOcupadas(t.id),0)
 ...
 const turmasLotadas=turmasVisiveis.filter(t=>turmaLotada(t)).length
 const turmasResumo=turmasVisiveis.map(t=>{
...
async function salvarUsuarioPerfil(e){
  e.preventDefault()
  if(!isAdmin)return falhar('Somente administradores podem modificar acessos.')
  ...
  const payload={
    ...
    professor_id:usuarioForm.professor_id||null
  }
 ...
 useEffect(()=>{
  if(!session||!isProfessor)return
  const ops=opcoesRelatorioPermitidas()
  if(ops.length>0&&!ops.some(o=>o.value===tipoRelatorio)){
    setTipoRelatorio(ops[0].value)
  }
},[session,perfilUsuario,turmas,usuarios])
 ...
 function relatorioPermitido(){
  ...
  return String(tipoRelatorio||'').startsWith('turma_') && opcoesRelatorioPermitidas().some(o=>o.value===tipoRelatorio)
 }

 function opcoesRelatorioPermitidas(){
  if(isProfessor){
    return turmasVisiveis.map(t=>({
      value:'turma_'+t.id,
      label:`${nomeTurma(t)} (${vagasDisponiveis(t)} vagas disponíveis)`
    }))
  }
  return [
    {value:'matriculados',label:'Alunos matriculados'},
    {value:'cadastrados',label:'Alunos cadastrados'},
    {value:'sem_matricula',label:'Alunos sem matrícula'},
    {value:'inativos',label:'Alunos inativos'},
    {value:'excluidos',label:'Alunos excluídos'},
 ...
 const opcoesRelatorio=opcoesRelatorioPermitidas()
 ...
 return <main data-aba={aba} className={isProfessor?"app-shell professor-readonly-shell":"app-shell"}><header className="app-header-fixed premium-header">
      ...
      </header><nav className="tabs">{abas.filter(([id])=>podeVerAba(id)).map(([id,n])=><button key={id} className={aba===id?'active':''} onClick={()=>setAba(id)}>{n}</button>)}</nav>{loading&&<p className="loading-premium">Atualizando dados...</p>}
 {(msg||erro)&&<div className="toast-container"><div className={erro?'toast error':'toast success'}>{erro||msg}</div></div>}{!podeVerAba(aba)&&<section className="card"><h2>Acesso restrito</h2><p className="muted">Seu perfil não tem permissão para visualizar este módulo.</p></section>}{aba==='painel'&&<section className="dashboard-inteligente">
  ...
  <form className="modal-card" onSubmit={confirmarRenovacao}>
    ...
    {!renovacaoForm.manter_turma&&<>
      ...
      <select value={renovacaoForm.turma_id} onChange={e=>setRenovacaoForm({...renovacaoForm,turma_id:e.target.value})} required>
        ...
        {turmasVisiveis.map(t=><option key={t.id} value={t.id} disabled={renovacaoForm.periodo_letivo_id&&turmaLotada(t,renovacaoForm.periodo_letivo_id)}>{t.cursos?.nome} - {t.nome} ({renovacaoForm.periodo_letivo_id?vagasDisponiveis(t,renovacaoForm.periodo_letivo_id):vagasDisponiveis(t)} vagas disponíveis)</option>)}
      </select>
    </>}
