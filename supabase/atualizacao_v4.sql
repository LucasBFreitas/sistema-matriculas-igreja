...
alter table alunos add column if not exists excluido boolean default false;
update alunos set ativo = true where ativo is null;
update alunos set excluido = false where excluido is null;
create table if not exists professores (id uuid primary key default gen_random_uuid(), nome text not null, cpf text, telefone text, email text, ativo boolean default true, created_at timestamptz default now());
