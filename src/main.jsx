...
function App(){
 ...
 const [buscaAluno,setBuscaAluno]=useState(''),[filtroAluno,setFiltroAluno]=useState('todos'),[buscaInscricao,setBuscaInscricao]=useState(''),[relatorioTurma,setRelatorioTurma]=useState('matriculados'),[modalRelatorio,setModalRelatorio]=useState(false)
...
async function confirmarRenovacao(e){
 ...
 if(!session)return <main className="login-page premium-login-page"><section className="login-hero-panel"><div className="login-hero-content"><img src="/logo-viva-esperanca.png" alt="Logo Viva Esperança"/><h2>Projeto Viva Esperança</h2><p>Gestão simples, acolhedora e organizada para matrículas do projeto social.</p></div></section><form className="card login-card premium-login-card" onSubmit={login}><h1>Bem-vindo ao Sistema Viva Esperança</h1>
        <p>Gerencie alunos, matrículas, presenças e relatórios em um só lugar.</p><input placeholder="E-mail" value={email} onChange={e=>setEmail(e.target.value)} type="email" required/><input placeholder="Senha" value={senha} onChange={e=>setSenha(e.target.value)} type="password" required/><button>Entrar</button></form></main>
 ...
 {aba==='relatorios'&&<section className="card"><div className="form-title"><h2>Relatórios</h2><button onClick={()=>setModalRelatorio(true)}>Imprimir relatório</button></div><select value={relatorioTurma} onChange={e=>setRelatorioTurma(e.target.value)}><option value="matriculados">Alunos matriculados</option><option value="ativos">Alunos ativos</option><option value="sem_matricula">Alunos sem matrícula</option><option value="inativos">Alunos inativos</option><option value="excluidos">Alunos excluídos</option>{turmas.map(t=><option key={t.id} value={`turma:${t.id}`}>{t.cursos?.nome} - {t.nome} ({vagasDisponiveis(t)} vagas disponíveis)</option>)}</select><div id="relatorio-impressao"><h1>Relatório de Alunos</h1><table><thead><tr><th>Aluno</th><th>Turma</th><th>Período</th><th>Professor</th><th>Presenças</th></tr></thead><tbody>{relatorio.map(r=><tr key={r.id}><td>{r.aluno}</td><td>{r.turma}</td><td>{r.periodo||'-'}</td><td>{r.professor}</td><td>{r.presencas||0}</td></tr>)}</tbody></table></div></section>}
