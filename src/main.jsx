...
async function confirmarRenovacao(e){
 ...
 {aba==='relatorios'&&<section className="card"><div className="form-title"><h2>Relatórios</h2><button onClick={()=>setModalRelatorio(true)}>Imprimir relatório</button></div><select value={relatorioTurma} onChange={e=>setRelatorioTurma(e.target.value)}><option value="matriculados">Alunos matriculados</option><option value="ativos">Alunos cadastrados</option><option value="sem_matricula">Alunos sem matrícula</option><option value="inativos">Alunos inativos</option><option value="excluidos">Alunos excluídos</option>{turmas.map(t=><option key={t.id} value={`turma:${t.id}`}>{t.cursos?.nome} - {t.nome} ({vagasDisponiveis(t)} vagas disponíveis)</option>)}</select><div id="relatorio-impressao"><h1>Relatório de Alunos</h1><table><thead><tr><th>Aluno</th><th>Turma</th><th>Período</th><th>Professor</th><th>Presenças</th></tr></thead><tbody>{relatorio.map(r=><tr key={r.id}><td>{r.aluno}</td><td>{r.turma}</td><td>{r.periodo||'-'}</td><td>{r.professor}</td><td>{r.presencas||0}</td></tr>)}</tbody></table></div></section>}
